...
def generate_patient(x, v, num_treatments, treatment_selection_bias, dosage_selection_bias,
    ...
    return treatment, dosages[treatment], outcomes[treatment] + np.random.normal(0, noise_std),outcomes
...
def get_dataset_splits(dataset):
    dataset_keys = ['x', 't', 'd', 'y','o','y_normalized']
    #i=0
    #m=0
    ...
    dataset_test['p'] = []
    dataset_test['o'] = []
    for t in dataset_train['t']:
        if t == 0:
            dataset_train['p'].append(dataset_train['y'][i])
        elif t == 1:
            dataset_train['o'].append(dataset_train['y'][i])
        i=i+1
    dataset_train['p'] = np.array(dataset_train['p'])
    dataset_train['o'] = np.array(dataset_train['o'])
    for t in dataset_test['t']:
        if t == 0:
            dataset_test['p'].append(dataset_test['y'][m])
        elif t == 1:
            dataset_test['o'].append(dataset_test['y'][m])
        m=m+1
    dataset_test['p'] = np.array(dataset_test['p'])
    dataset_test['o'] = np.array(dataset_test['o'])'''
    #print(dataset_test['o'].shape)
...
class TCGA_Data():
    def __init__(self, args):
        ...
        #self.tcga_data = np.loadtxt("data/Twin_data.csv", delimiter=",",skiprows=1,encoding = 'utf-8')

        # Define features
        #self.patients = self.normalize_data(self.tcga_data[:,:30])

        self.tcga_data = pickle.load(open('datasets/tcga.p', 'rb'))
        self.patients = self.normalize_data(self.tcga_data['rnaseq'])
        col_rand_array = np.arange(self.patients.shape[1])
        np.random.shuffle(col_rand_array)
        row_rand = self.patients[:,col_rand_array[0:300]]
        self.patients=row_rand
    ...
    def generate_dataset(self, patient_features, num_treatments):
        ...
        for patient in patient_features:
            t, dosage, y,potential= generate_patient(x=patient, v=self.v, num_treatments=num_treatments,
            ...
            tcga_dataset['o'].append(potential)
            '''if t==0:
                tcga_dataset['p'].append(y)
            elif t==1:
                tcga_dataset['o'].append(y)

        print(tcga_dataset['p'].__len__())
        print(tcga_dataset['o'].__len__())'''

        for key in ['x', 't', 'd', 'y','o']:
            tcga_dataset[key] = np.array(tcga_dataset[key])
        #print(tcga_dataset['o'].shape)
